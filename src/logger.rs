...
macro_rules! evtcall_format_buffer_log {
	($buf:expr,$len:expr,$info:tt,$iv:expr,$($arg:tt)+) => {
		...
		c.push_str(&evtcall_log_get_timestamp());
		c.push_str(": ");
		c.push_str(&(format!($($arg)+)[..]));
...
macro_rules! evtcall_debug_buffer_error {
	($buf:expr,$len:expr,$($arg:tt)+) => {
		evtcall_format_buffer_log!($buf,$len,"<ERROR>",0,$($arg)+);
	}
}
...
macro_rules! evtcall_debug_buffer_warn {
	($buf:expr,$len:expr,$($arg:tt)+) => {
		evtcall_format_buffer_log!($buf,$len,"<WARN>",10,$($arg)+);
	}
}
...
macro_rules! evtcall_debug_buffer_info {
	($buf:expr,$len:expr,$($arg:tt)+) => {
		evtcall_format_buffer_log!($buf,$len,"<INFO>",20,$($arg)+);
	}
}
...
macro_rules! evtcall_debug_buffer_debug {
	($buf:expr,$len:expr,$($arg:tt)+) => {
		evtcall_format_buffer_log!($buf,$len,"<DEBUG>",30,$($arg)+);
	}
}
